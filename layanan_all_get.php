...
$sql = mysql_query("SELECT layanan.*,pasien.*,posyandu.*,users.*, pasien.nama AS nama_pasien, users.nama AS nama_petugas FROM layanan INNER JOIN pasien ON layanan.id_pas=pasien.id_pas INNER JOIN posyandu ON pasien.id_pos=posyandu.id_pos INNER JOIN users ON posyandu.id_petugas=users.id_users ORDER BY id_layanan DESC")or die(mysql_error());
